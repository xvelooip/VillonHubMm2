--[[
    VILLONHUB TERMINAL ENGINE – Custom New UI Edition (Updated with VisualWorld & Sliders)
    Версия с полностью сохранённым функционалом VillonHub, слайдерами и вкладкой VisualWorld.
]]
...
local Options = {
    ...
    VisualWorld = {
        ...
        JumpCircleColor = Color3.fromRGB(0, 150, 255),
        JumpCircleRadius = 5,
        TrailEnabled = false,
        TrailColor = Color3.fromRGB(255, 0, 100),
        TrailLifetime = 0.5
    }
}
...
--// Состояние Lighting до изменения Fog
local DefaultFog = {
...
-- Jump Circle Functionality
local function spawnJumpCircle()
    if not Options.VisualWorld.JumpCircleEnabled then return end
    local char = LocalPlayer.Character
    ...
    circle.Size = Vector3.new(0.1, 0.05, 0.1)
    circle.CFrame = CFrame.new(root.Position - Vector3.new(0, 2.8, 0))
    circle.Material = Enum.Material.Neon
    circle.Color = Options.VisualWorld.JumpCircleColor
    circle.Parent = Workspace

    local cylinder = Instance.new("SpecialMesh")
    cylinder.MeshType = Enum.MeshType.Cylinder
    cylinder.Parent = circle

    local targetRadius = Options.VisualWorld.JumpCircleRadius
    TweenService:Create(circle, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = Vector3.new(0.05, targetRadius * 2, targetRadius * 2),
        Transparency = 1
    }):Play()
...
local attachment0, attachment1 = nil, nil

local function updateTrailState()
    local char = LocalPlayer.Character
    local root = getRoot(char)
    
    if Options.VisualWorld.TrailEnabled and root then
        if not activeTrail or not activeTrail.Parent then
            ...
            activeTrail.Attachment0 = attachment0
            activeTrail.Attachment1 = attachment1
            activeTrail.FaceCamera = true
            activeTrail.Color = ColorSequence.new(Options.VisualWorld.TrailColor)
            activeTrail.Lifetime = Options.VisualWorld.TrailLifetime
...
local MasterRenderLoop = RunService.RenderStepped:Connect(function()
    ...
    -- Обновление Fog в VisualWorld
    if Options.VisualWorld.FogEnabled then
        Lighting.FogStart = Options.VisualWorld.FogStart
        Lighting.FogEnd = Options.VisualWorld.FogEnd
        Lighting.FogColor = Options.VisualWorld.FogColor
    end
...
local function createSlider(parent, text, min, max, configTable, configKey, callback)
    local frame = Instance.new("Frame")
    ...
    valueLabel.Text = tostring(Options[configTable][configKey])
    ...
    local startPct = (Options[configTable][configKey] - min) / (max - min)
    fill.Size = UDim2.new(math.clamp(startPct, 0, 1), 0, 1, 0)
    fill.BackgroundColor3 = Color3.fromRGB(85, 85, 95)
    ...
    local function updateSlider(input)
        ...
        Options[configTable][configKey] = val
...
createSlider(combatScroll, "FOV Radius Size", 10, 500, "Aimbot", "Radius", function(val)
...
-- 4. VISUALWORLD (НОВАЯ ВКЛАДКА)
createToggle(visualWorldScroll, "Enable Custom Fog", "VisualWorld", "FogEnabled", function(state)
    if not state then
        Lighting.FogStart = DefaultFog.Start
        Lighting.FogEnd = DefaultFog.End
        Lighting.FogColor = DefaultFog.Color
    end
end)
createSlider(visualWorldScroll, "Fog Start Distance", 0, 500, "VisualWorld", "FogStart")
createSlider(visualWorldScroll, "Fog End Distance", 100, 2000, "VisualWorld", "FogEnd")

createToggle(visualWorldScroll, "Enable Jump Circle", "VisualWorld", "JumpCircleEnabled")
createSlider(visualWorldScroll, "Jump Circle Radius", 2, 15, "VisualWorld", "JumpCircleRadius")

createToggle(visualWorldScroll, "Enable Character Trail", "VisualWorld", "TrailEnabled", function()
    updateTrailState()
end)
createSlider(visualWorldScroll, "Trail Lifetime (x10 sec)", 1, 30, "VisualWorld", "TrailLifetime", function(val)
    Options.VisualWorld.TrailLifetime = val / 10
    updateTrailState()
end)
...
createSlider(miscScroll, "WalkSpeed Value", 16, 200, "Misc", "WalkSpeedValue")
...
createSlider(miscScroll, "Fov Value", 30, 120, "Misc", "FovValue")
createToggle(miscScroll, "Speed Glitch (BHOP)", "Misc", "SpeedGlitchEnabled")
createSlider(miscScroll, "Speed Glitch Value", 1, 50, "Misc", "SpeedGlitchValue")
...
do
    ...
    createExecuteButton(changeFlingScroll, "FLING SELECTED", function()
        if #selectedPlayers == 0 then
            showVillonNotice("Выберите игроков для флинга!")
            return
        end
        showVillonNotice("Запуск флинга по выбранным игрокам...")
        task.spawn(function()
            ...
            showVillonNotice("Флинг выбранных игроков завершен!")
        end)
    end)

    createExecuteButton(changeFlingScroll, "STOP FLING", function()
        if #selectedPlayers == 0 then
            showVillonNotice("Выберите игроков для остановки!")
            return
        end
        for _, p in ipairs(selectedPlayers) do
            if p and p.Character then
    ...
    playerListContainer.Size = UDim2.new(1, -8, 0, 0)
    playerListContainer.AutomaticSize = Enum.AutomaticSize.Y
    playerListContainer.BackgroundTransparency = 1
    playerListContainer.Parent = changeFlingScroll
...
showVillonNotice("VillonHub с новым UI и VisualWorld запущен!")
print("=== VillonHub Loaded with New UI & VisualWorld Tab ===")