--[[
    ...
    WindUI, с исправленными конфигами, change fling, unload, без aspect ratio
]]
...
-- // Хранилище позиций кнопок (для сохранения в конфиг)
...
--// Создаём ScreenGui для биндов (как в deepko.lua)
...
-- Мастер-рендер для кнопок и ESP (как в deepko.lua)
local MasterRenderLoop = RunService.RenderStepped:Connect(function()
    ...
    if Options.Aimbot.BindButtonEnabled then
        if not BindGui:FindFirstChild("Aim_BindButton") then
            ...
            BindGui.Aim_BindButton.Visible = true
            BindGui.Aim_BindButton.Draggable = IsAimBindEditModeActive
            BindGui.Aim_BindButton.BackgroundColor3 = Options.Aimbot.Enabled and Color3.fromRGB(0, 180, 255) or Color3.fromRGB(40, 40, 45)
            -- обновляем сохранённую позицию, если она изменилась
            local pos = BindGui.Aim_BindButton.Position
...
--// Misc (без AspectRatio, добавили Unload)
...
--// Configs (с сохранением и загрузкой позиций кнопок)
do
    ...
    TabConfigs:Button({
        ...
        Callback = function()
            ...
            -- Загружаем позиции кнопок и применяем их
...
--// Change Fling (как в deepko.lua — с галочками и кнопками)
do
    local flingTab = TabChangeFling
    -- Создаём ScrollingFrame внутри вкладки
    ...
    scroller.Parent = flingTab:GetElement()  -- контейнер вкладки

    local selectedPlayers = {}
    local checkBoxes = {}

    local function updateFlingPlayerList()
        -- Очищаем старые кнопки (кроме самого ScrollingFrame)
    ...
    -- Кнопки FLING и STOP внизу вкладки
    ...
    flingBtn.MouseButton1Click:Connect(function()
        ...
        for _, p in ipairs(selectedPlayers) do
            if p.Character then
                SHubFling(p)
                task.wait(1)  -- небольшая задержка между флингами
            end
        end
...
showVillonNotice("VillonHub загружен (WindUI, исправлены конфиги, change fling, unload)")
