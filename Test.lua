--[[
    VILLONHUB TERMINAL ENGINE – Кастомное GUI
    Полная версия: аим, ESP, шот мардер, флинг, KillAll, VisualWorld, ChangeFling, конфиги, растяг
    Стартовое окно с кнопкой Start, пузырь с перетаскиванием и кликом для открытия меню
]]
...
-- ============================================================
--   GUI – КАСТОМНОЕ (с исправленным стартовым окном и пузырём)
-- ============================================================
...
------------------------------------------------------------------
-- 1. INTRO (снежинки, VillonHub + Hello, кнопка Start)
------------------------------------------------------------------
...
titleLabel.Position = UDim2.new(0.5, -175, 0.38, 0)
titleLabel.BackgroundTransparency = 1
titleLabel.Text = "VillonHub"
titleLabel.Font = Enum.Font.GothamBold
titleLabel.TextSize = 42
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
titleLabel.TextTransparency = 0
titleLabel.ZIndex = 52
titleLabel.Parent = intro

local helloLabel = Instance.new("TextLabel")
helloLabel.Size = UDim2.new(0, 200, 0, 40)
helloLabel.Position = UDim2.new(0.5, -100, 0.53, 0)
helloLabel.BackgroundTransparency = 1
helloLabel.Text = "Hello"
helloLabel.Font = Enum.Font.GothamLight
helloLabel.TextSize = 28
helloLabel.TextColor3 = Color3.fromRGB(200, 200, 210)
helloLabel.TextTransparency = 0
helloLabel.ZIndex = 52
helloLabel.Parent = intro

local startBtn = Instance.new("TextButton")
startBtn.Size = UDim2.new(0, 140, 0, 42)
startBtn.Position = UDim2.new(0.5, -70, 0.65, 0)
...
------------------------------------------------------------------
-- 2. ПУЗЫРЬ (с кнопкой для открытия)
------------------------------------------------------------------
...
-- Кнопка для открытия меню (занимает всю правую часть)
local openBtn = Instance.new("TextButton")
openBtn.Size = UDim2.new(1, -28, 1, 0)
openBtn.Position = UDim2.new(0, 28, 0, 0)
openBtn.BackgroundTransparency = 1
openBtn.Text = ""
openBtn.ZIndex = 50
openBtn.Active = true
openBtn.Parent = bubble
openBtn.MouseButton1Click:Connect(function()
    print("[VillonHub] Bubble clicked!")
    if menuOpen then 
        closeMenu() 
    else 
        openMenu() 
...
-- ============================================================
-- ФУНКЦИИ СОЗДАНИЯ ЭЛЕМЕНТОВ GUI (createToggle, createSlider, createExecuteButton)
-- ============================================================
...
-- ============================================================
-- ЗАПОЛНЕНИЕ ПАНЕЛЕЙ (аим, ESP, MM2, Misc, ChangeFling, Configs, VisualWorld)
-- ============================================================
...
saveBtn.MouseButton1Click:Connect(function() saveConfig(nameBox.Text) end)
local loadBtn = createExecuteButton(configsScroll, "Load Config")
loadBtn.MouseButton1Click:Connect(function() loadConfig(nameBox.Text) end)
...
------------------------------------------------------------------
-- ОТКРЫТИЕ / ЗАКРЫТИЕ МЕНЮ (исправленное)
------------------------------------------------------------------
function openMenu()
    if menuOpen then return end
    menuOpen = true
    print("[VillonHub] openMenu called")
...
function closeMenu()
    ...
    menuOpen = false
    print("[VillonHub] closeMenu called")
...
------------------------------------------------------------------
-- ЗАПУСК (обработчик кнопки Start, исправленный)
------------------------------------------------------------------
startBtn.MouseButton1Click:Connect(function()
    print("[VillonHub] Start clicked")
    ...
    local introTween = TweenService:Create(intro, TweenInfo.new(0.7), {BackgroundTransparency = 1})
    local titleTween = TweenService:Create(titleLabel, TweenInfo.new(0.5), {TextTransparency = 1})
    local hello = TweenService:Create(helloLabel, TweenInfo.new(0.5), {TextTransparency = 1})
    local btnTween = TweenService:Create(startBtn, TweenInfo.new(0.5), {TextTransparency = 1, BackgroundTransparency = 1})
    local strokeTween = TweenService:Create(startStroke, TweenInfo.new(0.5), {Transparency = 1})
