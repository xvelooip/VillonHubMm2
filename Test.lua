...
local function createSlider(parent, text, value, callback)
    ...
    -- Упрощённый слайдер: клик по значению меняет его
...
------------------------------------------------------------------
-- ЗАПУСК ПОСЛЕ НАЖАТИЯ START (ИСПРАВЛЕННЫЙ)
------------------------------------------------------------------
startBtn.MouseButton1Click:Connect(function()
    print("[VillonHub] Start button clicked!")  -- отладка
    ...
    if snowConnection then
        ...
        print("[VillonHub] Snow connection disconnected")
    end

    -- Прячем intro с анимацией
    local introTween = TweenService:Create(intro, TweenInfo.new(0.7), {BackgroundTransparency = 1})
    local titleTween = TweenService:Create(title, TweenInfo.new(0.5), {TextTransparency = 1})
    ...
    -- Ждём завершения основной анимации
    introTween.Completed:Wait()
    print("[VillonHub] Intro hidden")

    -- Уничтожаем intro и всё, что внутри
    intro:Destroy()
    print("[VillonHub] Intro destroyed")
