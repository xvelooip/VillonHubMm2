--[[
    VILLONHUB TERMINAL ENGINE – WindUI
    Тема: Dark Violet
    Полная версия: Combat, Visuals, MM2 (RoleESP с настройками), Misc (включая растяг), Configs, ChangeFling, VisualWorld
    Шот мардер – новый (кеш, предрасчёт, асинхронный выстрел)
]]
...
-- ============================================================
--   НОВЫЙ ШОТ МАРДЕР (с кешем, предрасчётом, асинхронным выстрелом)
-- ============================================================

-- Кеш ролей для шот мардера (обновляется в фоне)
local smCachedRoles = {}
task.spawn(function()
    while true do
...
-- Предварительное вычисление цели (обновляется в фоне)
...
-- Функция выстрела (асинхронная, использует кешированную цель)
...
-- ===== СТАРАЯ ФУНКЦИЯ getMurdererTarget (для AutoAimMurder, остаётся для совместимости) =====
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if Options.VisualsWorld.TrailEnabled then toggleTrail(true) end
    if Options.VisualsWorld.ChinaHatEnabled then toggleChinaHat(true) end
    if Options.VisualsWorld.FogEnabled then toggleFog(true) end
end)
...
-- ============================================================
--   ОСТАЛЬНЫЕ ФУНКЦИИ (флинг, киллы, аим, ESP)
-- ============================================================
...
local MasterRenderLoop = RunService.RenderStepped:Connect(function()
    ...
    if Options.Aimbot.BindButtonEnabled then
        ...
        if BindGui:FindFirstChild("Aim_BindButton") then BindGui.Aim_BindButton.Visible = false end
    end
    ...
    if Options.MM2.TPGunButtonEnabled then
        ...
        if BindGui:FindFirstChild("MM2_TPGunButton") then BindGui.MM2_TPGunButton.Visible = false end
    end
    ...
    if Options.MM2.InvisButtonEnabled then
        ...
        if BindGui:FindFirstChild("MM2_InvisButton") then BindGui.MM2_InvisButton.Visible = false end
    end
    ...
    if Options.MM2.FlingButtonEnabled then
        ...
        if BindGui:FindFirstChild("MM2_FlingButton") then BindGui.MM2_FlingButton.Visible = false end
    end

    -- SHOOT MURDER (использует новый шот мардер)
    if Options.MM2.ShootMurderButtonEnabled then
        ...
        if BindGui:FindFirstChild("MM2_ShootMurderButton") then BindGui.MM2_ShootMurderButton.Visible = false end
    end
    ...
    if Options.MM2.ThrowKnifeButtonEnabled then
        ...
        if BindGui:FindFirstChild("MM2_ThrowKnifeButton") then BindGui.MM2_ThrowKnifeButton.Visible = false end
    end
    ...
    if Options.MM2.KillAllButtonEnabled then
        ...
        if BindGui:FindFirstChild("KillAllButton") then BindGui.KillAllButton.Visible = false end
    end
    ...
    if Options.MM2.KillSheriffButtonEnabled then
        ...
        if BindGui:FindFirstChild("KillSheriffButton") then BindGui.KillSheriffButton.Visible = false end
    end
    ...
    for player, storage in pairs(VisualStorage) do
        ...
        if char then
            ...
            if root and head and human and human.Health > 0 then
                local isRole = roleEnabled and cachedRoles[player.Name] and cachedRoles[player.Name] ~= "Default"
                local currentBoxColor = Options.Visuals.BoxColor
                local roleColor = GetMM2RoleColor(player)
                if isRole then
                    currentBoxColor = roleColor
                end

                local isTeammate = Options.Visuals.TeamCheck and player.Team == LocalPlayer.Team and not isRole
                if isTeammate then
                ...
                -- Чамсы / Обводка для RoleESP
                if isRole and (roleSettings.Chams or roleSettings.Outline) then
                    ...
                    if roleSettings.Chams and roleSettings.Outline then
                        storage.Highlight.FillColor = roleColor
                        storage.Highlight.FillTransparency = 0.4
                        storage.Highlight.OutlineColor = roleColor
                        storage.Highlight.OutlineTransparency = 0
                    elseif roleSettings.Chams then
                        storage.Highlight.FillColor = roleColor
                        storage.Highlight.FillTransparency = 0.4
                        storage.Highlight.OutlineTransparency = 1
                    elseif roleSettings.Outline then
                        storage.Highlight.FillColor = roleColor
                        storage.Highlight.FillTransparency = 1
                        storage.Highlight.OutlineColor = roleColor
                        storage.Highlight.OutlineTransparency = 0
                    end
                    storage.Highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
                    storage.Highlight.Enabled = true
                elseif Options.Visuals.Chams and not isRole then
                    if not storage.Highlight then
                        storage.Highlight = Instance.new("Highlight")
                        storage.Highlight.Parent = CoreGui
                    end
                    storage.Highlight.Adornee = char
                    storage.Highlight.FillColor = Options.Visuals.ChamsColor
                    storage.Highlight.FillTransparency = 0.4
                    storage.Highlight.OutlineTransparency = 0
                ...
                if rootOn and headOn and legOn then
                    ...
                    -- Boxes для RoleESP
                    if isRole and roleSettings.Boxes then
                        storage.Box.Size = Vector2.new(bWidth, bHeight)
                        storage.Box.Position = Vector2.new(rootPos.X - bWidth / 2, headPos.Y)
                        storage.Box.Color = roleColor
                        storage.Box.Visible = true
                    elseif Options.Visuals.Boxes and not isRole then
                        storage.Box.Size = Vector2.new(bWidth, bHeight)
                        storage.Box.Position = Vector2.new(rootPos.X - bWidth / 2, headPos.Y)
                        storage.Box.Color = Options.Visuals.BoxColor
                        storage.Box.Visible = true
                    else
                        storage.Box.Visible = false
                    end

                    -- Names для RoleESP (новый формат)
                    if isRole and roleSettings.Names then
                        local roleName = cachedRoles[player.Name] or "Innocent"
                        storage.Name.Text = ("%s (%s)"):format(player.Name, roleName)
                        storage.Name.Color = roleColor
                        storage.Name.Position = Vector2.new(rootPos.X, headPos.Y - 15)
                        storage.Name.Visible = true
                    elseif Options.Visuals.Names and not isRole then
                    ...
                    -- HealthBar (только для обычного ESP)
                    if Options.Visuals.HealthBar and not isRole then
                    ...
                    -- Tracers (только для обычного ESP)
                    if Options.Visuals.Tracers and not isRole then
                    ...
                    -- Skeletons для RoleESP
                    if isRole and roleSettings.Skeletons then
                        if char:FindFirstChild("LeftUpperArm") and char:FindFirstChild("RightUpperArm") then
                    ...
                    elseif Options.Visuals.Skeletons and not isRole then
                        if char:FindFirstChild("LeftUpperArm") and char:FindFirstChild("RightUpperArm") then
                            local function GetBoneVector(partName)
                                local p = char[partName]
                                local pos, on = Camera:WorldToViewportPoint(p.Position)
...
-- ============================================================
--   РАСТЯГ (включение/выключение через RenderStepped)
-- ============================================================
...
-- ============================================================
--   GUI – WINDUI (темно-фиолетовая тема)
-- ============================================================

local WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua"))()

-- Создаём окно с темой "Dark" и затем переопределяем цвета
...
-- Изменяем цвета вручную (темно-фиолетовый)
local function applyVioletTheme()
    local mainFrame = Window:GetElement()
    if mainFrame then
        mainFrame.BackgroundColor3 = Color3.fromRGB(26, 10, 46) -- #1a0a2e
        local titleBar = mainFrame:FindFirstChild("TitleBar")
        if titleBar then
            titleBar.BackgroundColor3 = Color3.fromRGB(50, 20, 80)
            local titleText = titleBar:FindFirstChild("Title")
            if titleText then
                titleText.TextColor3 = Color3.fromRGB(180, 130, 255)
            end
        end
        -- Изменяем цвет кнопки закрытия (X) и других элементов при необходимости
        for _, child in ipairs(mainFrame:GetDescendants()) do
            if child:IsA("TextButton") and child.Name == "CloseButton" then
                child.TextColor3 = Color3.fromRGB(255, 100, 200)
            end
            if child:IsA("TextButton") and child.Name == "MinimizeButton" then
                child.TextColor3 = Color3.fromRGB(200, 150, 255)
            end
        end
    end
    -- Изменяем цвет фона вкладок (контейнер)
    local content = Window:GetContent()
    if content then
        content.BackgroundColor3 = Color3.fromRGB(20, 8, 40)
    end
end
...
-- Переопределяем цвет кнопки открытия меню (она находится в верхнем левом углу)
-- В WindUI это обычно кнопка с иконкой "≡", её можно найти как "MenuButton"
local function recolorMenuButton()
    local mainFrame = Window:GetElement()
    if mainFrame then
        local menuBtn = mainFrame:FindFirstChild("MenuButton")
        if menuBtn and menuBtn:IsA("TextButton") then
            menuBtn.TextColor3 = Color3.fromRGB(180, 130, 255)
            menuBtn.BackgroundColor3 = Color3.fromRGB(50, 20, 80)
        end
    end
end
task.wait(0.2)
...
-- Для доступа к кнопке открытия меню, если она создаётся позже, можно использовать повторную проверку
local menuButtonCheck = RunService.RenderStepped:Connect(function()
    ...
    if mainFrame then
        ...
        if menuBtn and menuBtn:IsA("TextButton") then
            menuBtn.TextColor3 = Color3.fromRGB(180, 130, 255)
            menuBtn.BackgroundColor3 = Color3.fromRGB(50, 20, 80)
            menuButtonCheck:Disconnect()
        end
    end
end)
table.insert(Connections, menuButtonCheck)
...
local TabVisualWorld = Window:Tab({ Title = "VisualWorld" })

--// Функции создания элементов WindUI с фиолетовой подсветкой
local function CreateToggle(tab, text, configTable, configKey, subKey)
    local toggle = tab:Toggle({
        Title = text,
        Desc = "",
        Value = subKey and Options[configTable][configKey][subKey] or Options[configTable][configKey],
        Callback = function(state)
            if subKey then
                Options[configTable][configKey][subKey] = state
            else
                Options[configTable][configKey] = state
            end
            if configKey == "Enabled" and configTable == "Aimbot" then
            ...
            if configTable == "MM2" and configKey == "Fling" then
                showVillonNotice(state and "Touch Fling Mode ENABLED" or "Touch Fling Mode DISABLED")
            end
            if configTable == "VisualsWorld" then
                if configKey == "TrailEnabled" then toggleTrail(state)
                elseif configKey == "FogEnabled" then toggleFog(state)
                elseif configKey == "JumpCircleEnabled" then toggleJumpCircle(state)
                elseif configKey == "ChinaHatEnabled" then toggleChinaHat(state) end
            end
            if configTable == "Misc" and configKey == "StretchEnabled" then
                updateStretch()
            end
        end
    })
    -- Изменяем цвет активного состояния на фиолетовый
    local toggleElement = toggle:GetElement()
    if toggleElement then
        local knob = toggleElement:FindFirstChild("Knob")
        if knob then
            knob.BackgroundColor3 = Color3.fromRGB(120, 40, 200)
        end
    end
    return toggle
end

local function CreateNumericInput(tab, text, configTable, configKey, subKey, min, max)
    local input = tab:Input({
        Title = text,
        Desc = "",
        Value = subKey and tostring(Options[configTable][configKey][subKey]) or tostring(Options[configTable][configKey]),
        Placeholder = "Значение",
        Callback = function(value)
            local num = tonumber(value)
            if num then
                if subKey then
                    Options[configTable][configKey][subKey] = num
                else
    ...
    local inputElement = input:GetElement()
    if inputElement then
        local textBox = inputElement:FindFirstChild("TextBox")
        if textBox then
            textBox.BackgroundColor3 = Color3.fromRGB(40, 20, 60)
            textBox.TextColor3 = Color3.fromRGB(200, 180, 255)
        end
    end
    return input
end
...
--   ВКЛАДКА VisualWorld
-- ============================================================
CreateToggle(TabVisualWorld, "Trail (шлейф)", "VisualsWorld", "TrailEnabled")
CreateToggle(TabVisualWorld, "Fog (туман)", "VisualsWorld", "FogEnabled")
CreateToggle(TabVisualWorld, "Jump Circle (кольцо при прыжке)", "VisualsWorld", "JumpCircleEnabled")
CreateToggle(TabVisualWorld, "China Hat (конус над головой)", "VisualsWorld", "ChinaHatEnabled")

CreateNumericInput(TabVisualWorld, "China Hat Size", "VisualsWorld", "ChinaHatSize", nil, 1.5, 8)
CreateNumericInput(TabVisualWorld, "Trail Lifetime (длина)", "VisualsWorld", "TrailLifetime", nil, 0.1, 2)
CreateNumericInput(TabVisualWorld, "Fog Distance (густота)", "VisualsWorld", "FogDistance", nil, 20, 800)

-- Кнопка палитры цветов (кубики) с фиолетовым фоном
local colorPaletteOpen = false
local colorPaletteGui = nil

local function createColorPalette()
    if colorPaletteGui then
        colorPaletteGui.Visible = true
        return
    end

    local gui = Instance.new("ScreenGui")
    gui.Name = "ColorPalette"
    ...
    for _, color in ipairs(colors) do
        ...
        btn.MouseButton1Click:Connect(function()
            Options.VisualsWorld.Color = color
            updateAllWorldEffects()
            showVillonNotice("Цвет изменён")
            gui.Visible = false
        end)
    end
...
TabVisualWorld:Button({
    Title = "Open Color Palette",
    Callback = function()
        createColorPalette()
        if colorPaletteGui then
            colorPaletteGui.Visible = not colorPaletteGui.Visible
        end
    end
})

-- ============================================================
--   ОСТАЛЬНЫЕ ВКЛАДКИ (Combat, Visuals, MM2, Misc, Configs, ChangeFling)
-- ============================================================
-- (Они такие же, как в предыдущей версии, с фиолетовым акцентом, но я повторю их для полноты)
...
CreateNumericInput(TabAimbot, "FOV Radius Size", "Aimbot", "Radius", nil, 1, 500)
...
-- Вкладка MM2
CreateToggle(TabMM2, "Role ESP (on/off)", "MM2", "RoleESP", "Enabled")
TabMM2:Label("Role ESP Settings")
CreateToggle(TabMM2, "  Boxes", "MM2", "RoleESP", "Boxes")
CreateToggle(TabMM2, "  Skeletons", "MM2", "RoleESP", "Skeletons")
CreateToggle(TabMM2, "  Names (Name + Role)", "MM2", "RoleESP", "Names")
CreateToggle(TabMM2, "  Chams (fill)", "MM2", "RoleESP", "Chams")
CreateToggle(TabMM2, "  Outline (edge)", "MM2", "RoleESP", "Outline")
TabMM2:Label("")
...
TabMM2:Button({
    Title = "Fling Murderer (one time)",
    Callback = function() FlingMurderer() end
})
TabMM2:Button({
    Title = "Fling Sheriff (one time)",
...
CreateNumericInput(TabMisc, "WalkSpeed Value", "Misc", "WalkSpeedValue", nil, 1, 100)
...
CreateNumericInput(TabMisc, "Fov Value", "Misc", "FovValue", nil, 1, 120)
CreateToggle(TabMisc, "Speed Glitch (BHOP)", "Misc", "SpeedGlitchEnabled")
CreateNumericInput(TabMisc, "Speed Glitch Value", "Misc", "SpeedGlitchValue", nil, 1, 50)
CreateToggle(TabMisc, "Stretch (растяг)", "Misc", "StretchEnabled")
...
TabMisc:Button({
    ...
    Callback = function()
        ...
        Window:Destroy()
        BindGui:Destroy()
        if colorPaletteGui then colorPaletteGui:Destroy() end
...
do
    ...
    scroller.Parent = flingTab:GetElement()
    ...
    local function updateFlingPlayerList()
        ...
        scroller.CanvasSize = UDim2.new(0, 0, 0, y + 10)
    end
    ...
    buttonFrame.Position = UDim2.new(0, 5, 0.87, 0)
    buttonFrame.BackgroundTransparency = 1
    buttonFrame.Parent = flingTab:GetElement()
...
showVillonNotice("VillonHub загружен (Dark Violet Theme)")
