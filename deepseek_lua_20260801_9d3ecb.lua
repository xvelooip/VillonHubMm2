--[[
    VILLONHUB TERMINAL ENGINE – Custom New UI Edition (исправленный)
    Вкладки: Combat, Visuals, MM2, Misc, ChangeFling
    В каждой вкладке левая и правая колонка со своим содержимым.
]]
...
------------------------------------------------------------------
-- 3. ОСНОВНОЕ МЕНЮ (с двумя колонками и вкладками)
------------------------------------------------------------------
...
-- Контент (левая и правая колонки)
...
-- Функция создания панели для вкладки
local function createTabPanel(column, titleText)
...
local tabNames = {"Combat", "Visuals", "MM2", "Misc", "ChangeFling"}
local leftPanels = {}
local rightPanels = {}
local leftScrolls = {}
local rightScrolls = {}
...
for _, name in ipairs(tabNames) do
    local lp, ls, lt = createTabPanel(leftColumn, name)
...
-- Делаем видимой первую вкладку (Combat)
...
local function fillMM2()
    ...
    -- Правая колонка – только кнопки Fling/Kill
...
-- 5. CHANGEFLING
local function fillChangeFling()
    local ls = leftScrolls["ChangeFling"]
    local rs = rightScrolls["ChangeFling"]
    -- Левая: список игроков с галочками
    ...
    -- Правая: кнопки FLING SELECTED и STOP FLING
    createExecuteButton(rs, "FLING SELECTED", function()
    ...
    createExecuteButton(rs, "STOP FLING", function()
        if #selectedPlayers == 0 then
            showVillonNotice("Выберите игроков")
            return
        end
...
local function switchTab(name)
    ...
    -- Скрываем старые панели, показываем новые
    for _, t in pairs(tabNames) do
        if t == name then
            leftPanels[t].Visible = true
            rightPanels[t].Visible = true
        else
            leftPanels[t].Visible = false
            rightPanels[t].Visible = false
        end
    end

    -- Обновляем заголовки правой колонки (для каждой вкладки свои)
    local rightTitlesMap = {
        Combat = "Actions",
        Visuals = "ESP",
        MM2 = "Kills",
        Misc = "Utils",
    ...
    rightTitles[name].Text = rightTitlesMap[name] or ""
    ...
    -- Прокручиваем левую и правую панели вверх
    local leftScroll = leftScrolls[name]
    local rightScroll = rightScrolls[name]
    if leftScroll then
        TweenService:Create(leftScroll, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {CanvasPosition = Vector2.new(0, 0)}):Play()
    end
    if rightScroll then
        TweenService:Create(rightScroll, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {CanvasPosition = Vector2.new(0, 0)}):Play()
    end
end
