...
-- ===== ОБНОВЛЁННАЯ getMurdererTarget (улучшенный предикшен) =====
...
MainFrame.Size = UDim2.new(0, 500, 0, 315) -- уменьшен
...
local MasterRenderLoop = RunService.RenderStepped:Connect(function()
    ...
    -- КНОПКА SHOOT MURDER (обновлённая, с улучшенным предикшеном)
...
do
    ...
    local function updateCoinCache()
        local now = os.clock()
        if now - CoinCacheTime < 0.8 then return end
        CoinCacheTime = now
        CachedCoins = {}
        for _, obj in ipairs(Workspace:GetDescendants()) do
    ...
    local function updateBagStatus()
        local now = os.clock()
        if now - BagCheckTime < 0.8 then return end
        BagCheckTime = now
        BagFull = false
...
print("VillonHub обновлён. Добавлены: улучшенный шот мардер, новый автофарм, удалены анимации.")