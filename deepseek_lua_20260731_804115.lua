...
-- ============================================================
--   НОВЫЙ ПОЛЬЗОВАТЕЛЬСКИЙ ИНТЕРФЕЙС (из Updui.txt)
-- ============================================================
...
leftTitle.Size = UDim2.new(1, -16, 0, 24)
leftTitle.Position = UDim2.new(0, 12, 0, 6)
leftTitle.BackgroundTransparency = 1
leftTitle.Text = "Combat"
leftTitle.Font = Enum.Font.GothamBold
leftTitle.TextSize = 14
leftTitle.TextColor3 = Color3.fromRGB(235, 235, 240)
leftTitle.TextXAlignment = Enum.TextXAlignment.Left
leftTitle.Parent = leftColumn

local leftScroll = Instance.new("ScrollingFrame")
leftScroll.Size = UDim2.new(1, -12, 1, -36)
leftScroll.Position = UDim2.new(0, 6, 0, 32)
leftScroll.BackgroundTransparency = 1
leftScroll.BorderSizePixel = 0
leftScroll.ScrollBarThickness = 3
leftScroll.ScrollBarImageColor3 = Color3.fromRGB(70, 70, 80)
...
rightTitle.Size = UDim2.new(1, -16, 0, 24)
rightTitle.Position = UDim2.new(0, 12, 0, 6)
rightTitle.BackgroundTransparency = 1
rightTitle.Text = "Actions"
rightTitle.Font = Enum.Font.GothamBold
rightTitle.TextSize = 14
rightTitle.TextColor3 = Color3.fromRGB(235, 235, 240)
rightTitle.TextXAlignment = Enum.TextXAlignment.Left
rightTitle.Parent = rightColumn

local rightScroll = Instance.new("ScrollingFrame")
rightScroll.Size = UDim2.new(1, -12, 1, -36)
rightScroll.Position = UDim2.new(0, 6, 0, 32)
rightScroll.BackgroundTransparency = 1
rightScroll.BorderSizePixel = 0
rightScroll.ScrollBarThickness = 3
rightScroll.ScrollBarImageColor3 = Color3.fromRGB(70, 70, 80)
...
------------------------------------------------------------------
-- Вкладки + анимации (переименованы)
------------------------------------------------------------------
...
local tabNames = {"Combat", "Visuals", "MM2", "Misc"} -- убрали Pose и Other
local tabButtons = {}
...
local function switchTab(name)
    ...
    TweenService:Create(leftColumn, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
        Position = UDim2.new(-0.1, 0, 0, 0),
        BackgroundTransparency = 0.4
    }):Play()
    TweenService:Create(rightColumn, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    ...
    task.delay(0.15, function()
        ...
        elseif name == "Visuals" then
            leftTitle.Text = "Visuals"
            rightTitle.Text = "ESP"
        elseif name == "MM2" then
            leftTitle.Text = "MM2"
            rightTitle.Text = "Kills"
        elseif name == "Misc" then
            leftTitle.Text = "Misc"
            rightTitle.Text = "Utils"
        end

        leftColumn.Position = UDim2.new(-0.1, 0, 0, 0)
        rightColumn.Position = UDim2.new(0.6, 0, 0, 0)
        leftColumn.BackgroundTransparency = 0.4
        rightColumn.BackgroundTransparency = 0.4

        TweenService:Create(leftColumn, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Position = UDim2.new(0, 0, 0, 0),
            BackgroundTransparency = 0
        }):Play()
        TweenService:Create(rightColumn, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
-- ============================================================
-- ЗАПОЛНЕНИЕ ВКЛАДОК (все опции из frek.lua)
-- ============================================================

-- 1. COMBAT (левая колонка)
createToggle(leftScroll, "Enable Hard Aim", Options.Aimbot.Enabled, function(state)
    Options.Aimbot.Enabled = state
    FOVCircle.Visible = state
end)
createToggle(leftScroll, "Show Aim Bind Button", Options.Aimbot.BindButtonEnabled)
createExecuteButton(leftScroll, "Edit AIM Button Position", function()
...
createToggle(leftScroll, "Team Check", Options.Aimbot.TeamCheck)
createToggle(leftScroll, "Wall Check", Options.Aimbot.WallCheck)
createSlider(leftScroll, "FOV Radius", 10, 500, Options.Aimbot.Radius, function(val)
    Options.Aimbot.Radius = val
    FOVCircle.Radius = val
end)
-- Правая колонка: кнопки
createExecuteButton(rightScroll, "Shoot Murderer", shootMurderer)

-- 2. VISUALS
createToggle(leftScroll, "Boxes ESP", Options.Visuals.Boxes)
createToggle(leftScroll, "Skeletons ESP", Options.Visuals.Skeletons)
createToggle(leftScroll, "Chams Style", Options.Visuals.Chams)
createToggle(leftScroll, "Names Rendering", Options.Visuals.Names)
createToggle(leftScroll, "Health Bar", Options.Visuals.HealthBar)
createToggle(leftScroll, "Snaplines", Options.Visuals.Tracers)
createToggle(leftScroll, "Team Check Filter", Options.Visuals.TeamCheck)

-- 3. MM2
createToggle(leftScroll, "Role ESP", Options.MM2.RoleESP)
createToggle(leftScroll, "Dropped Gun ESP", Options.MM2.GunESP)
createToggle(leftScroll, "Show TP Gun Button", Options.MM2.TPGunButtonEnabled)
createExecuteButton(leftScroll, "Edit TP GUN Position", function()
...
createToggle(leftScroll, "Show Shoot Murder Button", Options.MM2.ShootMurderButtonEnabled)
createExecuteButton(leftScroll, "Edit SHOOT MURDER Position", function()
...
createToggle(leftScroll, "Show Throw Knife Button", Options.MM2.ThrowKnifeButtonEnabled)
createExecuteButton(leftScroll, "Edit THROW KNIFE Position", function()
...
createToggle(leftScroll, "Show Invis Bind Button", Options.MM2.InvisButtonEnabled)
createExecuteButton(leftScroll, "Edit INVIS Position", function()
...
createToggle(leftScroll, "Show Fling Bind Button", Options.MM2.FlingButtonEnabled)
createExecuteButton(leftScroll, "Edit FLING Position", function()
...
createToggle(leftScroll, "Aim Murderer Only", Options.MM2.AimMurderOnly)
createToggle(leftScroll, "Auto-Aim murder", Options.MM2.AutoAimMurder)
createToggle(leftScroll, "Invisibility", Options.MM2.Invisibility, function(state)
    toggleInvisibilityLogic(state)
end)
createToggle(leftScroll, "Touch Fling", Options.MM2.Fling, function(state)
    showVillonNotice(state and "Touch Fling Mode ACTIVE" or "Touch Fling Mode DISABLED")
end)
createToggle(leftScroll, "Auto TP Gun on Drop", Options.MM2.AutoTPGun)
createToggle(leftScroll, "Show Kill All Button", Options.MM2.KillAllButtonEnabled)
createExecuteButton(leftScroll, "Edit KILL ALL Position", function()
...
createToggle(leftScroll, "Show Kill Sheriff Button", Options.MM2.KillSheriffButtonEnabled)
createExecuteButton(leftScroll, "Edit KILL SHERIFF Position", function()
...
createToggle(leftScroll, "Jerk Off", Options.MM2.JerkOffEnabled)

-- Правая колонка: кнопки Kill/FLING
createExecuteButton(rightScroll, "Fling Murderer", FlingMurderer)
createExecuteButton(rightScroll, "Fling Sheriff", FlingSheriff)
createExecuteButton(rightScroll, "Kill All", KillAll)
createExecuteButton(rightScroll, "Kill Sheriff", KillSheriff)

-- 4. MISC (левая колонка)
createToggle(leftScroll, "Spin Bot", Options.Misc.SpinBot)
createToggle(leftScroll, "Custom WalkSpeed", Options.Misc.WalkSpeedEnabled)
createSlider(leftScroll, "WalkSpeed Value", 16, 200, Options.Misc.WalkSpeedValue, function(val)
    Options.Misc.WalkSpeedValue = val
end)
createToggle(leftScroll, "Anti-Fling Protection", Options.Misc.AntiFling)
createToggle(leftScroll, "Double Jump", Options.Misc.DoubleJump)
createToggle(leftScroll, "Change Fov", Options.Misc.FovEnabled)
createSlider(leftScroll, "Fov Value", 30, 120, Options.Misc.FovValue, function(val)
    Options.Misc.FovValue = val
end)
createToggle(leftScroll, "Speed Glitch (BHOP)", Options.Misc.SpeedGlitchEnabled)
createSlider(leftScroll, "Speed Glitch Value", 1, 50, Options.Misc.SpeedGlitchValue, function(val)
    Options.Misc.SpeedGlitchValue = val
end)
createToggle(leftScroll, "Stretch (растяг)", Options.Misc.StretchEnabled)
-- Aspect Ratio
createToggle(leftScroll, "Aspect Ratio", Options.Misc.AspectRatioEnabled, function(state)
    Options.Misc.AspectRatioEnabled = state
    updateAspectRatio()
end)
createSlider(leftScroll, "Aspect Ratio Value", 0.5, 1.5, Options.Misc.AspectRatioValue, function(val)
...
-- Правая колонка: Unload
createExecuteButton(rightScroll, "UNLOAD CHEAT", function()
...
-- Configs (на правой панели, переделаем как отдельные кнопки)
...
createExecuteButton(rightScroll, "Save Config", function()
...
createExecuteButton(rightScroll, "Load Config", function()
    local name = nameBox.Text
    if name == "" then
        showVillonNotice("Введите имя конфига")
        return
...
-- Change Fling (в правой колонке, с галочками и кнопками)
...
flingContainer.Parent = rightScroll
