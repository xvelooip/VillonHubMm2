...
-- ===== НАСТРОЙКИ АДАПТИВНОГО ПРЕДИКШЕНА =====
local CONFIG_SM = {
    BulletSpeed = 300,
...
-- ===== ОПТИМИЗИРОВАННАЯ getMurdererTarget (использует cachedRoles) =====
            ...
            if player and player ~= LocalPlayer then
                ...
                if char then
                    ...
                    if hrp then
                        ...
                        table.insert(velocityHistory, {pos = currentPos, vel = velocity, time = os.clock()})
                        if #velocityHistory > CONFIG_SM.HistorySize then
                        ...
                        if #velocityHistory >= 2 then
                            for i = 1, #velocityHistory do
                                avgVel = avgVel + velocityHistory[i].vel
                            end
                            avgVel = avgVel / #velocityHistory
                        else
                            avgVel = velocity
                        end
                        ...
                        if #velocityHistory >= 3 then
                            local v1 = velocityHistory[#velocityHistory-1].vel
                            local v2 = velocityHistory[#velocityHistory].vel
                            local dt = velocityHistory[#velocityHistory].time - velocityHistory[#velocityHistory-1].time
                            if dt > 0.001 then
                        ...
                        local predictedPos = currentPos
                            + avgVel * travelTime * CONFIG_SM.PredictionFactor
                            + 0.5 * acceleration * travelTime * travelTime * 0.5

                        if math.abs(avgVel.Y) > 5 then
                            predictedPos = predictedPos + Vector3.new(0, avgVel.Y * travelTime * CONFIG_SM.VerticalAim, 0)
                        end

                        local diff = predictedPos - currentPos
                        if diff.Magnitude > CONFIG_SM.MaxPrediction then
                            predictedPos = currentPos + diff.Unit * CONFIG_SM.MaxPrediction
                        end
...
local function throwKnifeToClosest()
    ...
    if myRoot then
        ...
        if prediction.Magnitude > CONFIG_SM.MaxPrediction then
            prediction = prediction.Unit * CONFIG_SM.MaxPrediction
        end
...
local function throwKnifeAtTarget(targetPlayer)
    ...
    if myRoot then
        local dist = (targetPos - myRoot.Position).Magnitude
        local travelTime = dist / 300
        local prediction = vel * travelTime * 0.85
        if prediction.Magnitude > 300 then
            prediction = prediction.Unit * 300
        end
        targetPos = targetPos + prediction
    end
    local knifeCFrame = knife:FindFirstChild("Handle") and knife.Handle.CFrame or knife.CFrame
...
MainFrame.Size = UDim2.new(0, 500, 0, 315) -- уменьшен на 10%
...
CreateTabButton("AUTOFARM", 6, "AutoFarm")
    ...
    if Options.MM2.ShootMurderButtonEnabled then
        if not ScreenGui:FindFirstChild("MM2_ShootMurderButton") then
            ...
            ShootMurderButton.MouseButton1Click:Connect(function()
                ...
                local targetCFrame = CFrame.new(targetPos)
                pcall(function()
                    shoot:FireServer(gunCFrame, targetCFrame)
                end)
            end)
        else
...
--// ВКЛАДКА AUTOFARM (оптимизированная)
do
    local farmTab = Tabs.AutoFarm
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Size = UDim2.new(0.9, 0, 0.12, 0)
    toggleBtn.Position = UDim2.new(0.05, 0, 0.02, 0)
    toggleBtn.BackgroundColor3 = getgenv().AutoFarm and Color3.fromRGB(50, 180, 80) or Color3.fromRGB(180, 50, 50)
    toggleBtn.Text = getgenv().AutoFarm and "AutoFarm: ON" or "AutoFarm: OFF"
    ...
    speedLabel.Position = UDim2.new(0.05, 0, 0.16, 0)
    ...
    speedInput.Size = UDim2.new(0.25, 0, 0.08, 0)
    speedInput.Position = UDim2.new(0.7, 0, 0.16, 0)
    speedInput.BackgroundColor3 = Color3.fromRGB(35, 35, 42)
    speedInput.Text = tostring(getgenv().FarmSpeed or 35)
    ...
    -- ОПТИМИЗИРОВАННАЯ ЛОГИКА АВТОФАРМА (снижена частота обновлений)
    local velocity = nil
    local currentTarget = nil
    local isInLobbyFlag = false
    local lastActiveTime = os.clock()

    local LobbyParts = {}
    local LobbyCacheTime = 0
    local function updateLobbyCache()
        local now = os.clock()
        if now - LobbyCacheTime < 5 then return end
        LobbyCacheTime = now
        LobbyParts = {}
        ...
        if #LobbyParts == 0 then
            local loadLobby = Workspace:FindFirstChild("LoadLobby")
            if loadLobby then
                for _, part in ipairs(loadLobby:GetDescendants()) do
                    if part:IsA("BasePart") then
                        table.insert(LobbyParts, part)
                    end
                end
        ...
        if #LobbyParts == 0 then
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("BasePart") and (obj.Name:lower():find("lobby") or obj.Name:lower():find("load")) then
                    table.insert(LobbyParts, obj)
                end
            end
        end
    end
    ...
    local function isInLobby()
        if #LobbyParts == 0 then return false end
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if not root then return false end
        for _, part in ipairs(LobbyParts) do
            if (root.Position - part.Position).Magnitude < 30 then
                return true
            end
        end
        return false
    ...
    local function antiAfk(root)
        if not root then return end
        local now = os.clock()
        if velocity and velocity.Velocity.Magnitude < 0.5 then
            if now - lastActiveTime > 60 then
                local forward = root.CFrame.LookVector
                root.CFrame = root.CFrame + forward * 1
                lastActiveTime = now
            end
        else
            lastActiveTime = now
    ...
    local function updateCoinCache()
        local now = os.clock()
        if now - CoinCacheTime < 0.8 then return end  -- реже обновляем (было 0.3)
        CoinCacheTime = now
        CachedCoins = {}
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("BasePart") and obj.Transparency < 1 then
                local name = obj.Name
                if name == "MainCoin" or name == "Coin_Server" or name == "Coin" then
    ...
    local BagFull = false
    local BagCheckTime = 0
    local function updateBagStatus()
        local now = os.clock()
        if now - BagCheckTime < 0.8 then return end  -- реже
        BagCheckTime = now
        BagFull = false
        local pGui = LocalPlayer:FindFirstChild("PlayerGui")
        if pGui and pGui:FindFirstChild("MainGUI") then
            local gameGui = pGui.MainGUI:FindFirstChild("Game")
            if gameGui and gameGui:FindFirstChild("CoinBag") then
                local bagContainer = gameGui.CoinBag:FindFirstChild("Container") or gameGui.CoinBag:FindFirstChild("Amount")
                if bagContainer and bagContainer:IsA("TextLabel") then
                    local text = bagContainer.Text or ""
                    if string.find(text, "40") or string.find(text, "FULL") or string.find(text, "40/40") then
                        BagFull = true
                    end
                end
            end
    ...
    RunService.Heartbeat:Connect(function()
        if getgenv().AutoFarm and LocalPlayer.Character then
    ...
    task.spawn(function()
        while true do
            task.wait(0.1)  -- увеличено с 0.05 для снижения нагрузки
            ...
            isInLobbyFlag = isInLobby()
            if isInLobbyFlag then
            ...
            if BagFull then
                ...
                currentTarget = nil
                antiAfk(root)
                statusLabel.Text = "[ СУМКА ПОЛНА ]"
                statusLabel.TextColor3 = Color3.fromRGB(255, 170, 0)
            elseif #CachedCoins == 0 then
                velocity.Velocity = Vector3.zero
                currentTarget = nil
                antiAfk(root)
                statusLabel.Text = "[ НЕТ МОНЕТ ]"
                statusLabel.TextColor3 = Color3.fromRGB(100, 180, 255)
            else
                local target = nil
                local minDist = math.huge
                for _, c in pairs(CachedCoins) do
                ...
                if target and target.Parent then
                    ...
                    if dist > 1.5 then
                        ...
                        velocity.Velocity = dir * getgenv().FarmSpeed
                        currentTarget = target
                        lastActiveTime = os.clock()
                        statusLabel.Text = "[ ЛЕТИМ К МОНЕТЕ ]"
                        statusLabel.TextColor3 = Color3.fromRGB(80, 255, 120)
                    else
                        local coinPos = target.Position
                        local coinRef = target
                        root.CFrame = CFrame.new(coinPos + Vector3.new(0, 0.1, 0))
                        task.wait(0.05)
                        if coinRef and coinRef.Parent and coinRef.Transparency < 1 then
                            root.CFrame = coinRef.CFrame
                            task.wait(0.05)
                        end
                        velocity.Velocity = Vector3.zero
                        currentTarget = nil
...
print("VillonHub обновлён. Удалены анимации, убран текст Jerk Off, уменьшено меню, оптимизирован автофарм.")